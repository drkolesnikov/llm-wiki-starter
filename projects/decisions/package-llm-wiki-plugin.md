...
Ship an installable `llm-wiki` Python CLI and a local Codex plugin scaffold. The CLI initializes a visible namespaced `llm-wiki/` workspace inside a target repo and adds only a small root `AGENTS.md` pointer block outside that namespace.

## Rationale

- A console script gives humans and agents one stable command: `llm-wiki init <repo>`.
- A visible `llm-wiki/` directory keeps generated wikis discoverable in macOS Finder while staying namespaced.
